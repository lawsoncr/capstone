...
public class EnemyAlienAI : MonoBehaviour
{
    ...
    private void ChasePlayer() {
        if (!isDead && !isPlayerDead) {
            ...
            animator.SetBool("alienFlight", true);
        }
        if (isPlayerDead) { agent.ResetPath(); animator.SetBool("alienIdle", true); return; }
    }
        ...
        if (!isDead && !isPlayerDead) {
            ...
            if (!attackedAlready) {
                    if (playerInAttackRange) {
                        animator.SetBool("alienAttack", true);
                        target.GetComponent<ThirdPersonMovement>().DamagePlayer(enemyDamage);
                        Debug.Log("Alien Attacked Player");
    ...
    private void ResetAttack() {
        ...
        animator.SetBool("alienAttack", false);
    }
    ...
    public void KillEnemy() {
        isDead = true;
        animator.SetBool("alienDead", true);
        animator.SetBool("alienFlight", false);
